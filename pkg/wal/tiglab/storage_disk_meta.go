...
func (file *LogFile) loadLogFileMetaHead(buf []byte) error {
	if len(buf) < LogMetaHeadSize {
		return ErrBufferSizeNotEnough
	}
	if binary.LittleEndian.Uint32(buf) != LogMagic {
		return ErrBadMagic
	}
	...
	if file.entryNum < 0 || file.entryNum > LogMaxEntryNum {
		return ErrBadMeta
	}
	return nil
}
func (file *LogFile) loadLogFileMeta(buf []byte) error {
	if len(buf) < LogFileMetaSize {
		return ErrBufferSizeNotEnough
	}
	...
	n := LogMetaHeadSize + file.entryNum*LogFileMetaEntrySize
	if cc := crc32.ChecksumIEEE(buf[8:n]); cc != crc {
		return ErrCrcNotMatch
	}
	for i := 0; i < file.entryNum; i++ {
		off := LogMetaHeadSize + i*LogFileMetaEntrySize
...
func (file *LogFile) getEntryMeta(pos int) (index, term uint64, offset int64, e error) {
	if pos >= 0 && pos < file.entryNum {
		...
		e = ErrIndexOutOfFileRange
	}
	return
}
func (file *LogFile) setEntryMeta(index, term uint64, offset int64, pos int) error {
	if pos < 0 && pos >= LogMaxEntryNum {
		return ErrIndexOutOfFileRange
	}
...
func LoadHardStateFile(dir string, shardId uint64) (*HardStateFile, error) {
	...
	for _, fi := range fis {
		...
		if len(b) < LogHsHeadSize {
			info = append(info, fmt.Sprintf("read size too small, %d", len(b)))
			continue
		}
		if binary.LittleEndian.Uint32(b[0:]) != LogMagic {
		...
		if LogHsHeadSize+hsLen+snapLen != len(b) {
			info = append(info, fmt.Sprintf("read %v short of data, %d, %d", fi.Name(), LogHsHeadSize+hsLen+snapLen, len(b)))
			continue
		}
		if e = json.Unmarshal(b[LogHsHeadSize:LogHsHeadSize+hsLen], hs); e != nil {
			info = append(info, fmt.Sprintf("read %v Unmarshal hs failed, %v", fi.Name(), e))
			continue
		}
		if e = json.Unmarshal(b[LogHsHeadSize+hsLen:], snap); e != nil {
...
func copySnapshotMeta(src, dst *proto.SnapshotMeta) *proto.SnapshotMeta {
	dst.Index = src.Index
	dst.Term = src.Term
	dst.Peers = make([]string, len(src.Peers))
