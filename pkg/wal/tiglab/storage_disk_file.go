...
func NewLogFile(dir string, shardId uint64, entries []*proto.Entry) (file *LogFile, n int, e error) {
	...
	file.entryMeta = make([]EntryMeta, LogMaxEntryNum)
	file.offset = LogFileMetaSize
...
func LoadLastLogFile(dir string, shardId uint64) (file *LogFile, e error) {
	...
	if _, e = file.f.ReadAt(mBuf[:LogMetaHeadSize], 0); e != nil {
		return cleanFunc(e)
	}
	...
	for file.entryNum < LogMaxEntryNum {
		var n int
		n, e = readEntry(entry, mBuf, buf, file.f, file.offset)
		if e == io.EOF || e == ErrCrcNotMatch {
			e = nil
			break
		}
		if e != nil {
...
func (file *LogFile) writeEntries(entries []*proto.Entry) (n int, e error) {
	...
	if file.entryNum > 0 && file.entryMeta[file.entryNum-1].Index+1 != entries[0].Index {
		return 0, ErrFileLastIndexNotMatch
	}
	cco := crc32.NewIEEE()
	head := entryHeadPool.GetBuffer()
	defer entryHeadPool.PutBuffer(head)

	for i := 0; i < len(entries) && entries[i].Index <= file.ranges[1] && file.entryNum < LogMaxEntryNum; i++ {
		entry := entries[i]
		...
		if _, e = file.f.WriteAt(head, file.offset); e == nil {
			_, e = file.f.WriteAt(entry.Data, file.offset+LogEntryHeadSize)
		}
		if e != nil {
			return n, e
		}
		...
		file.offset += int64(LogEntryHeadSize + len(entry.Data))
		file.entryNum++
		n++
	}
	return n, nil
...
func (file *LogFile) readEntries(lo, hi uint64, maxSize uint64) ([]*proto.Entry, uint64, bool, error) {
	...
	for i, pos := range poss {
		...
		if maxSize != math.MaxUint32 {
			if size > maxSize {
				return entries[:i], size - uint64(entry.Size()), true, nil
			}
			if size == maxSize {
...
func readEntry(entry *proto.Entry, headBuf, dataBuf []byte, r io.ReaderAt, pos int64) (int, error) {
	...
	if len(headBuf) < LogEntryHeadSize {
		headBuf = entryHeadPool.GetBuffer()
		defer entryHeadPool.PutBuffer(headBuf)
	}
	if n, e := r.ReadAt(headBuf[:LogEntryHeadSize], pos); e != nil {
		return n, e
	}
	...
	if n, e := r.ReadAt(entry.Data, pos+LogEntryHeadSize); e != nil {
		return LogEntryHeadSize + n, e
	}
	cco := crc32.NewIEEE()
	cco.Write(headBuf[4:])
	cco.Write(entry.Data)
	if cco.Sum32() != crc {
		return LogEntryHeadSize + length, ErrCrcNotMatch
	}
	return LogEntryHeadSize + length, nil
}
